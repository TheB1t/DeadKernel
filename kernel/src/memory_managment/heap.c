...
uint32_t heap_malloc(Heap_t* heap, uint32_t size, uint8_t align, uint32_t* phys) {
    if (heap) {
        ...
        if (phys) {
            Page_t* page = getPage(addr, 0, heap->pageDir);
...
static void expand(size_t newSize, Heap_t* heap) {
	...
	for (size_t i = oldSize; i < newSize; i += PAGE_SIZE)
		allocFrame(getPage(heap->startAddr + i, 1, heap->pageDir), heap->supervisor ? 1 : 0, heap->readonly ? 0 : 1);
...
static size_t contract(size_t newSize, Heap_t* heap) {
	...
	for (size_t i = oldSize - PAGE_SIZE; i >= newSize; i -= PAGE_SIZE)
		freeFrame(getPage(heap->startAddr + i, 0, heap->pageDir));
