...
uint32_t _kmalloc(uint32_t size, uint8_t align, uint32_t* phys) {
	if (kernelHeap) {
		uint32_t addr;
		if (align)
			addr = (uint32_t)palignedAlloc(size, kernelHeap);
		else
...
Heap_t* createHeap(uint32_t placementAddr, uint32_t start, uint32_t end, uint32_t max, uint8_t supervisor, uint8_t readonly) {
	...
	size_t holeSize = end - start;
	HeapChunk_t* hole = (HeapChunk_t*)start;
	hole->prevFoot = 0;
	set_size_and_pinuse(hole, holeSize);
...
static void expand(size_t newSize, Heap_t* heap) {
	...
	for (size_t i = oldSize; i < newSize; i += PAGE_SIZE)
		allocFrame(getPage(heap->startAddr + i, 1, kernelDir), heap->supervisor ? 1 : 0, heap->readonly ? 0 : 1);
...
static size_t contract(size_t newSize, Heap_t* heap) {
	...
	if (newSize < KHEAP_MIN_SIZE)
		newSize = KHEAP_MIN_SIZE;

	size_t oldSize = heap->endAddr - heap->startAddr;
	for (size_t i = oldSize - PAGE_SIZE; i >= newSize; i -= PAGE_SIZE)
