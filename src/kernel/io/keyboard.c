#include "keyboard.h"

#include "screen.h"

uint8_t kbdus[128] = {
	0,  27, '1', '2', '3', '4', '5', '6', '7', '8',
	'9', '0', '-', '=', '\b','\t',
	'q', 'w', 'e', 'r', 't', 'y', 'u', 'i', 'o', 'p', '[', ']', '\n', 0,/* 29   - Control */
	'a', 's', 'd', 'f', 'g', 'h', 'j', 'k', 'l', ';', '\'', '`', 0,	/* Left shift */
	'\\', 'z', 'x', 'c', 'v', 'b', 'n',	'm', ',', '.', '/', 0,	/* Right shift */
	'*',
	0,	/* Alt */
	' ',	/* Space bar */
	0,	/* Caps lock */
    0,	/* 59 - F1 key ... > */
    0,   0,   0,   0,   0,   0,   0,   0,
    0,	/* < ... F10 */
    0,	/* 69 - Num lock*/
    0,	/* Scroll Lock */
    0,	/* Home key */
    0,	/* Up Arrow */
    0,	/* Page Up */
  '-',
    0,	/* Left Arrow */
    0,
    0,	/* Right Arrow */
  '+',
    0,	/* 79 - End key*/
    0,	/* Down Arrow */
    0,	/* Page Down */
    0,	/* Insert Key */
    0,	/* Delete Key */
    0,   0,   0,
    0,	/* F11 Key */
    0,	/* F12 Key */
    0,	/* All other keys are undefined */
};

uint8_t init = 0;

uint32_t keyboardBufferSize = 0;
uint8_t* keyboardBuffer = 0x0;
uint8_t keyboardCursor = 0;

void putch(uint8_t ch) {
	if (keyboardCursor < keyboardBufferSize)
		keyboardBuffer[keyboardCursor++] = ch;
}

uint8_t getch() {
	while (keyboardCursor <= 0);
	return keyboardBuffer[--keyboardCursor];
}

uint8_t isKeyboardInit() {
	return init;
}

void keyboardHandler(CPURegisters_t* regs, uint32_t err_code) {
    uint8_t scancode = inb(0x60);
    
    if (scancode & 0x80) {

    } else {
        putch(kbdus[scancode]);
    }
}

uint8_t initKeyboard(uint32_t bufferSize) {
	ASSERT(bufferSize <= MAX_KEYBOARD_BUFFER_SIZE);
	keyboardBufferSize = bufferSize;
	keyboardBuffer = (uint8_t*)kmalloc(bufferSize);
	registerInterruptHandler(IRQ1, keyboardHandler);
	init = 1;
}
